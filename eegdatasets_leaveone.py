...
class EEGDataset:
    ...
    def __init__(
        ...
        if os.path.exists(features_filename):
            ...
            self.vlmodel, self.preprocess_train, self.feature_extractor = (
        ...
        for subject in self.subjects:
            if self.train:
                ...
                preprocessed_eeg_data = (
                    torch.from_numpy(data["preprocessed_eeg_data"]).float().detach()
                )
                ...
                for i in range(n_classes):
                    ...
                    # print("preprocessed_eeg_data_class", preprocessed_eeg_data_class.shape)
                    labels = torch.full(
                        (samples_per_class,), i, dtype=torch.long
                    ).detach()
                ...
                if subject == self.exclude_subject or self.exclude_subject == None:
                    ...
                    for i in range(n_classes):
                        start_index = (
                            i * samples_per_class
                        )  # Update start_index for each class
                        preprocessed_eeg_data_class = preprocessed_eeg_data[
                            start_index : start_index + samples_per_class
                        ]
                        # print("preprocessed_eeg_data_class", preprocessed_eeg_data_class.shape)
